import { NextRequest, NextResponse } from "next/server";
import { LEAGUE_KEYS, type LeagueKey } from "@/lib/sportsScores";
import { getTeamSchedule } from "@/lib/sportsTeams";

export const dynamic = "force-dynamic";

function isLeagueKey(value: string | null): value is LeagueKey {
  return value !== null && (LEAGUE_KEYS as string[]).includes(value);
}

export async function GET(req: NextRequest) {
  const league = req.nextUrl.searchParams.get("league");
  const teamId = req.nextUrl.searchParams.get("teamId");

  if (!isLeagueKey(league) || !teamId) {
    return NextResponse.json({ games: [], error: "Missing or unknown league/teamId" }, { status: 400 });
  }

  try {
    const games = await getTeamSchedule(league, teamId);
    return NextResponse.json({ games });
  } catch (err) {
    return NextResponse.json(
      { games: [], error: err instanceof Error ? err.message : "Unknown error" },
      { status: 200 }
    );
  }
}
