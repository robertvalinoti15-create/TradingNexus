...
export default function CryptoPage() {
  const { instruments, loading } = useTEInstruments("/api/crypto");
  const { paths, hydrated, addInstrument, removeInstrument } = useTEWatchlist(
    "stock-tracker:crypto-watchlist",
    DEFAULT_WATCHLIST
  );
  ...
  if (!hydrated) {
    return <div className="p-8 text-sm text-foreground/50">Loading your crypto watchlist…</div>;
  }
    ...
    <main className="max-w-5xl w-full mx-auto px-6 py-10 flex flex-col gap-10">
      ...
      <section className="flex flex-col gap-4">
        ...
        <AddTEInstrumentForm
          ...
          buttonLabel="Add crypto"
        />

        {loading && instruments.length === 0 && (
          <p className="text-sm text-foreground/50">Loading cryptocurrencies…</p>
        )}
        {!loading && watchlist.length === 0 && (
          <p className="text-sm text-foreground/50">Add a cryptocurrency above to start tracking it.</p>
        )}
        ...
        <div className="grid grid-cols-1 gap-6">
          {watchlist.map((c) => (
            <TEInstrumentCard
              key={c.path}
              instrument={c}
              tvSymbol={CRYPTO_TV_SYMBOLS[c.path]}
      ...
      <section>
        <h2 className="text-lg font-semibold mb-4">Biggest movers in crypto</h2>
        <div className="border border-foreground/10 rounded-xl p-4">
          <TEMoversTable instruments={instruments} loading={loading} />
        </div>
      </section>

      <MarketActivity market="crypto" />

      <section>
        <h2 className="text-lg font-semibold mb-4">All crypto prices</h2>
        <div className="border border-foreground/10 rounded-xl p-4">
